...
export function useCreateArchiveItem() {
  ...
  return useMutation({
    mutationFn: async (input: { title: string; content: string; attachments: ArchiveAttachment[]; createdBy: string }) => {
...
export function useUpdateArchiveItem() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: async (input: { id: string; title?: string; content?: string; attachments?: ArchiveAttachment[] }) => {
