...
// ---------------------------------------------------------------------------
// Employee Performance Tab (merged from EmployeePerformancePage)
// ---------------------------------------------------------------------------
  ...
  const { data: rawData, isLoading: dataLoading } = useQuery({
    ...
    queryFn: async () => {
      const [leadsRes, activitiesRes, contractsRes, meetingsRes, todosRes] =
        await Promise.all([
          supabase.from('leads').select('id, assigned_to, pipeline_stage'),
          supabase.from('lead_activities').select('id, created_by, activity_type'),
          supabase.from('contracts').select('id, sales_rep, service_rep, total_amount, status'),
          supabase.from('meetings').select('id, created_by'),
          supabase.from('todos').select('id, "ownerId", assignees, status, created_by'),
        ])
      return {
        leads: leadsRes.data || [],
  ...
  const metrics = useMemo<EmployeeMetrics[]>(() => {
    ...
    return profiles
      ...
      .map((profile) => {
        ...
        const leadsOwned = leads.filter((l: Record<string, unknown>) => l.assigned_to === uid).length
        const callsMade = activities.filter((a: Record<string, unknown>) => a.created_by === uid && a.activity_type === 'call').length
        const consultationsDone = activities.filter((a: Record<string, unknown>) => a.created_by === uid && a.activity_type === 'consultation').length
        const salesContracts = contracts.filter((c: Record<string, unknown>) => c.sales_rep === uid)
        const contractsAsSales = salesContracts.length
        const contractRevenueSales = salesContracts.reduce((sum: number, c: Record<string, unknown>) => sum + (Number(c.total_amount) || 0), 0)
        const contractsAsService = contracts.filter((c: Record<string, unknown>) => c.service_rep === uid).length
        const meetingsCreated = meetings.filter((m: Record<string, unknown>) => m.created_by === uid).length
        const ownedTodos = todos.filter((t: Record<string, unknown>) => t.ownerId === uid)
        const todosOwned = ownedTodos.length
        const todosCompleted = ownedTodos.filter((t: Record<string, unknown>) => t.status === 'done').length
        ...
        return { profile, leadsOwned, callsMade, consultationsDone, contractsAsSales, contractRevenueSales, contractsAsService, meetingsCreated, todosOwned, todosCompleted, todosAssigned }
      })
  }, [profiles, rawData])
...
function PerformanceTab({ t, selectedProfile }: { t: (key: string) => string; selectedProfile: string }) {
  const [deptFilter, setDeptFilter] = useState<string>('all')
  const { metrics, isLoading } = useEmployeePerformance()

  const filtered = useMemo(() => {
    let result = metrics
    if (deptFilter !== 'all') result = result.filter((m) => m.profile.department === deptFilter)
    if (selectedProfile !== 'all') result = result.filter((m) => m.profile.id === selectedProfile)
    return result
  }, [metrics, deptFilter, selectedProfile])
  ...
  const totals = useMemo(() => {
    return filtered.reduce(
      ...
      { employees: 0, leads: 0, calls: 0, consultations: 0, contracts: 0, revenue: 0, meetings: 0, todos: 0, todosComplete: 0 },
    )
  }, [filtered])
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        <Card>
          <CardContent className="p-3">
            <div className="flex items-center gap-2">
              <div className="rounded-full bg-blue-50 p-1.5"><Users className="h-3.5 w-3.5 text-blue-500" /></div>
              <div>
                <p className="text-[10px] font-medium text-muted-foreground uppercase">{t('empPerf.employees')}</p>
                <p className="text-lg font-bold">{totals.employees}{t('common.people')}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-3">
            <div className="flex items-center gap-2">
              <div className="rounded-full bg-green-50 p-1.5"><Phone className="h-3.5 w-3.5 text-green-500" /></div>
              <div>
                <p className="text-[10px] font-medium text-muted-foreground uppercase">{t('empPerf.totalCalls')}</p>
                <p className="text-lg font-bold">{totals.calls}{t('common.count')}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-3">
            <div className="flex items-center gap-2">
              <div className="rounded-full bg-violet-50 p-1.5"><TrendingUp className="h-3.5 w-3.5 text-violet-500" /></div>
              <div>
                <p className="text-[10px] font-medium text-muted-foreground uppercase">{t('empPerf.totalConsults')}</p>
                <p className="text-lg font-bold">{totals.consultations}{t('common.count')}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-3">
            <div className="flex items-center gap-2">
              <div className="rounded-full bg-amber-50 p-1.5"><FileText className="h-3.5 w-3.5 text-amber-500" /></div>
              <div>
                <p className="text-[10px] font-medium text-muted-foreground uppercase">{t('empPerf.totalContracts')}</p>
                <p className="text-lg font-bold">{totals.contracts}{t('common.count')}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-3">
            <div className="flex items-center gap-2">
              <div className="rounded-full bg-emerald-50 p-1.5"><FolderKanban className="h-3.5 w-3.5 text-emerald-500" /></div>
              <div>
                <p className="text-[10px] font-medium text-muted-foreground uppercase">{t('empPerf.projects')}</p>
                <p className="text-lg font-bold">{totals.todosComplete}/{totals.todos}</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      {/* Department filter (only when profile filter is 'all') */}
      {selectedProfile === 'all' && (
        <div className="flex items-center gap-3">
          <Select value={deptFilter} onValueChange={(v) => setDeptFilter(v || 'all')}>
            <SelectTrigger className="w-[160px] h-8">
              <span className="text-sm">{deptFilter === 'all' ? t('common.allDepartments') : t('dept.' + deptFilter)}</span>
            </SelectTrigger>
            <SelectContent>
      ...
      {/* Tables grouped by department */}
      {Array.from(grouped.entries()).map(([dept, members]) => (
        <Card key={dept}>
          <CardContent className="p-0">
            <div className="flex items-center gap-2 px-4 py-2.5 border-b bg-muted/30">
              <Badge className={dept !== 'none' ? DEPT_COLORS[dept as Department] : 'bg-gray-100 text-gray-600'}>
                {dept !== 'none' ? t('dept.' + dept) : t('empPerf.noDept')}
              </Badge>
              <span className="text-sm text-muted-foreground">{members.length}{t('common.people')}</span>
            </div>
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {members.map((m) => {
                  const hasActivity = m.leadsOwned > 0 || m.callsMade > 0 || m.consultationsDone > 0 || m.contractsAsSales > 0 || m.meetingsCreated > 0 || m.todosOwned > 0
                  return (
                    <TableRow key={m.profile.id} className={hasActivity ? '' : 'opacity-50'}>
                      <TableCell className="font-medium">{m.profile.name}</TableCell>
                      <TableCell><span className="text-xs text-muted-foreground">{m.profile.position || '-'}</span></TableCell>
                      <TableCell className="text-right tabular-nums">{m.leadsOwned || '-'}</TableCell>
                      <TableCell className="text-right tabular-nums">{m.callsMade || '-'}</TableCell>
                      <TableCell className="text-right tabular-nums">{m.consultationsDone || '-'}</TableCell>
                      <TableCell className="text-right tabular-nums">{m.contractsAsSales || '-'}</TableCell>
                      <TableCell className="text-right tabular-nums">{m.contractRevenueSales > 0 ? `${(m.contractRevenueSales / 10000).toFixed(0)}만` : '-'}</TableCell>
                      <TableCell className="text-right tabular-nums">{m.contractsAsService || '-'}</TableCell>
                      <TableCell className="text-right tabular-nums">{m.meetingsCreated || '-'}</TableCell>
                      <TableCell className="text-right tabular-nums">
                        {m.todosOwned > 0 ? <span><span className="text-green-600">{m.todosCompleted}</span>/{m.todosOwned}</span> : '-'}
                      </TableCell>
                    </TableRow>
                  )
                })}
      ...
      {filtered.length === 0 && (
        <div className="text-center py-12 text-muted-foreground">{t('empPerf.noEmployees')}</div>
      )}
    </div>
  )
}
    ...
    <div className="space-y-5">
      ...
      <Tabs value={tab} onValueChange={v => v && setTab(v)}>
        ...
        <TabsContent value="performance">
          <PerformanceTab t={t} selectedProfile={selectedProfile} />
        </TabsContent>
      </Tabs>
