...
필드 목록:
- parentName: 학부모/보호자의 실제 사람 이름 (성+이름). "어머니", "아버지", "(모)", "(부)" 등으로 표기된 사람의 이름을 추출. 반드시 사람 이름이어야 합니다.
- studentName: 학생(자녀)의 실제 사람 이름 (성+이름)
- meetingDate: 미팅 날짜 (YYYY-MM-DD 형식)
- meetingNumber: 몇 차 상담인지 (숫자만. 예: 1, 2, 3). 추정이 불가하면 null
- phone: 연락처/전화번호
- currentSchool: 현재 재학 중인 학교의 이름
...
7. 중요: "학원명", "학교명", "상담일", "연락처" 등의 필드 라벨/헤더를 값으로 추출하지 마세요. parentName과 studentName은 반드시 실제 한국인 이름(예: "김영희", "이준호")이어야 합니다. "학원명", "학부모명", "학생명" 같은 라벨은 이름이 아닙니다.
8. PDF 양식에서 "학원명: OO학원", "학부모명: 홍길동" 같은 패턴이 있으면, 콜론(:) 뒤의 실제 값만 추출하세요. 콜론 앞의 라벨을 값으로 사용하면 안 됩니다.`
